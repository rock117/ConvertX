...
pub fn convert_image(
    ...
    let final_img = if let (Some(w), Some(h)) = (options.width, options.height) {
        ...
        img.resize(w as u32, (w as f64 * img.height() as f64 / img.width() as f64) as u32, image::imageops::FilterType::Lanczos3)
    } else if let Some(h) = options.height {
        img.resize((h as f64 * img.width() as f64 / img.height() as f64) as u32, h as u32, image::imageops::FilterType::Lanczos3)
    } else {
        img
    };
