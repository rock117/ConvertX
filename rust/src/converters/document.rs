...
pub fn convert_document(
    ...
    let output_path = Path::new(output_dir).join(&output_name);
     ...
     if matches!(input_ext.as_str(), "md" | "markdown" | "docx" | "pptx")
         ...
         && !(matches!(input_ext.as_str(), "docx" | "pptx") && matches!(output_ext.as_str(), "docx" | "pptx"))
     {
...
fn convert_via_chromium(input_path: &str, output_path: &Path) -> ConvertResult {
    ...
    let checkbox_re = match regex::Regex::new(r#"(?i)<input[^>]*type=(?:\"checkbox\"|'checkbox')[^>]*>"#) {
        Ok(re) => re,
        Err(e) => {
            return ConvertResult {
                success: false,
...
fn convert_via_pandoc(input_path: &str, output_path: &Path) -> ConvertResult {
    ...
    match cmd.output()
    {
        ...
        Ok(out) => {
            ...
            if output_ext == "pdf" {
                if stderr_lc.contains("unicode character") && stderr_lc.contains("not set up for use with latex") {
                    message.push_str(
